...
#define SLA_R 0xA1     //Slaveadr. f�r Read:  1010 Postfix + A2 Pin 0 + 1 (Read)
#define SLA_W 0xA0     //Slaveadr. f�r Write: 1010 Postfix + A2 Pin 0 + 0 (Write)
...
int main() {
    ...
    UART::enableAsync();
    UART::setBaud(9600);
    UART::writeString("usart started\n");
    ...
    i2c_tx(0xFB, TW_WRITE_ACK); //Daten schreiben
    i2c_stop();
    ...
    i2c_start();
    i2c_tx(SLA_W, TW_SLAW_ACK);    //Sendewunsch.
    i2c_tx(MEM_ADR, TW_WRITE_ACK); //EEPROM Speicheradresse einstellen, aus der wir lesen m�chten
...
void i2c_init()
{
    ...
    TWBR = F_CPU / I2C_SCK - 16; //I2C_SCK ist die gew�nschte Frequenz. Eigentlich noch * Prescaler.
    //Der ist bei uns aber 1.

    UART::writeString("i2c initialized.\n");
...
void i2c_stop()
{
    ...
    PORTB=~TWSR;
    //Nicht auf TWINT pr�fen. Wird beim TWSTO nicht vom MC gesetzt!
}
